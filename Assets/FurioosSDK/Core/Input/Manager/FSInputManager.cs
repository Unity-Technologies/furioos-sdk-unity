...
namespace FurioosSDK.Core {
	public class FSInputManager  : FSManagerModule {

		public const int mouseToucheId = -150000;

		public delegate void FingerTouchEventHandler(FSInputTouch sender);
		...
		private List<FSInputTouch> aliveTouches;
		...
		private FSInputTouch defaultTouch;
		...
		private Dictionary<string,FSInputBinding> inputBindings = new Dictionary<string, FSInputBinding>();
		...
		public List<FSInputTouch> Touches {
			get {
				UpdateControlState();
				return aliveTouches;
			}
		...
		public void Start() { 
			...
			defaultTouch = new FSInputTouch();
			aliveTouches = new List<FSInputTouch>();
		...
		public void FSInputBinding(bool init = false) {
			if (init) {
				inputBindings.Add ("Move X", new FSInputBinding ());
				inputBindings.Add ("Move Y", new FSInputBinding ());
				inputBindings.Add ("Look X", new FSInputBinding ());
				inputBindings.Add ("Look Y", new FSInputBinding ());
				inputBindings.Add ("Cursor X", new FSInputBinding ());
				inputBindings.Add ("Cursor Y", new FSInputBinding ());
				inputBindings.Add ("Click", new FSInputBinding ());
				inputBindings.Add ("Jet Pack", new FSInputBinding ());
				inputBindings.Add ("Zoom", new FSInputBinding ());
			}
		...
		public FSInputTouch findOrCreateTouch(int touchId) {
			FSInputTouch fingerTouch = null;
			bool found = false;
			
			foreach(FSInputTouch testTouch in aliveTouches) {
		...
		private void UpdateControlState() {
			if(!isUpdated){
				...
				List<FSInputController> touchInputs = Manager.GetAllInstances<FSInputController>();


				foreach(FSInputController ti in touchInputs) {
				...
				if(aliveTouchesIds.Count==0) {
					FSInputTouch fingerTouch  = findOrCreateTouch(mouseToucheId);
				...
				if (numTouches == 2) {
					FSInputTouch touch1 = aliveTouches[0];
			        FSInputTouch touch2 = aliveTouches[1];
		...
		private void UpdateFSInputBinding() {
