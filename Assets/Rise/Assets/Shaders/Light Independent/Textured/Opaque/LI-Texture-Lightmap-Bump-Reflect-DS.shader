Shader "Observ3d/Light Independent/Textured/Opaque/LI-Texture-Lightmap-Bump-Reflect-DS" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			half4 frag (FragInput IN) : COLOR{
				...
				return half4((albedo.rgb * bump * (1.0-reflectQty)  + (albedo.rgb * (1-_ReflectColor) + reflectCol + 0.5) * reflectQty ) * lm,1.0);
			}
