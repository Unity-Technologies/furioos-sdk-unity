...
namespace Rise.Features.MovingMode {
	public class MovingModeFPS : RSCharacterMovingMode {
		...
		private float autoWalkStartTime = 0.0f;
		private float autoWalkTimeEnableDisable = 2.0f;

		private bool useJoysticks;

		private GameObject destMesh;

		public double testLatitude = 0;
		public double testLongitude = 0;

		private double lastTimeStamp;
		private double latitude;
		private double longitude;

		private bool lastUseGPSPositionning = false;
		...
		void OnGUI () {
			if(showDebugInfo){
				GUI.Label (new Rect (100, 100, 300, 200), string.Format("Latitude: {0}\nLongitude: {1}\nLocation service status: {2}\nIs Stuck: {3}\nSpeed: {4}\nPan: {5}\nTilt: {6}", latitude, longitude, Input.location.status, IsStuck, velocity.magnitude,Pan,Tilt));
			}
		}
		...
		void Update(){
			...
			if(useGPSPositionning){
				#if UNITY_EDITOR
						bool updateGPS = (latitude != testLatitude || longitude != testLongitude);
						...
						if(Input.location.status == LocationServiceStatus.Running){
							...
							updateGPS = Input.location.lastData.timestamp != lastTimeStamp;
		...
		void UpdateGPSState () {
			if(useGPSPositionning != lastUseGPSPositionning){
				if(useGPSPositionning){
					Debug.Log("GPS Started");
					Input.location.Start(1.0f, 0.25f);
				}else{
					Debug.Log("GPS Stopped");
					Input.location.Stop();
				}
				lastUseGPSPositionning = useGPSPositionning;
			}
		}
