...
namespace Rise.Core {
	public class RSOutputMode3DSplitted : RSOutputMode {

		public enum SplitMode { OverUnder, SideBySide};

		//public StereoscopicMode stereoscopicMode = StereoscopicMode.SideBySide ;
		...
		public GameObject[] objectsToHid;
		...
		public override void AttachToMovingMode(RSMovingMode movingMode){
			...
			if(movingMode!=null){
				...
				if(UiCamera != null && RSSceneManager.RenderMode != RenderModes.Cardboard) {
		...
		public override void DetachFromMovingMode(){
			...
			if(movingModeCamera!=null)movingModeCamera.enabled = true;
		...
		void Update() {
			...
			RSOutputModesManager omm = OutputModesManager;
		...
		public override void RenderImage(RenderTexture source, RenderTexture destination){
			...
			if(leftTexture!=null) Graphics.DrawTexture(leftRect,leftTexture);

			if(rightTexture!=null)Graphics.DrawTexture(rightRect,rightTexture);
		...
		public override void RenderGui(RenderTexture guiTexture){
			if (uiTexture == null || RSSceneManager.RenderMode == RenderModes.Cardboard)
				return;
			...
			if (SceneManager.IsMobileDevice) {
				if(InputManager.HasGoneActive) {
					lastTouchTime = Time.time;
				}

				if(Time.time - lastTouchTime > timeBeforeHidUIInMobile) return;
			}
