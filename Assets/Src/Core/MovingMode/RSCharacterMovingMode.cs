...
namespace Rise.Core {
	public class RSCharacterMovingMode : RSPanTiltMovingMode {
		...
		public void SetPosition(Vector3 pos){
			...
			if(characterController!=null)characterController.Move( velocity );
		...
		protected bool IsStuck {
			...
			private set{
				...
				if(!destPositionActiveIsStuck)destPositionActiveStuckTime = -1;
			}
		}
		...
		protected bool DestPositionActive {
			...
			private set{
				...
				if(!destPositionActive)IsStuck=false;
			}
		}
		...
		protected void UpdateDeltaSpeed(ref float frontDeltaSpeed,ref float straffDeltaSpeed, ref bool frontControlled, ref bool straffControlled){
			...
			if(DestPositionActive){
				...
				if(destDirection.x < 0)destPan = -destPan;
				...
				if(speed < 0.3f){
					if(!IsStuck){
						if(destPositionActiveStuckTime<0)destPositionActiveStuckTime = Time.time;
		...
		protected void UpdateCharacter(float frontDeltaSpeed, float straffDeltaSpeed, bool frontControlled, bool straffControlled){
			...
			if(TiltModified || RollModified){
				...
				if(characterCamera!=null)characterCamera.transform.localRotation = rotation;
			}
			...
			if(characterController!=null){
				...
				if(frontControlled){
					...
					if(velocity.z > 0.1f) velocity.z -= brakeDecceleration * Time.deltaTime;
					else if(velocity.z < -0.1f) velocity.z += brakeDecceleration * Time.deltaTime;
					else velocity.z = 0;
				}
				...
				if(straffControlled){
					...
					if(velocity.x > 0.1f) velocity.x -= brakeDecceleration * Time.deltaTime;
					else if(velocity.x < -0.1f) velocity.x += brakeDecceleration * Time.deltaTime;
					else velocity.x = 0;
				}
		
				
				if(!characterController.isGrounded)velocity.y -= 9.81f * Time.deltaTime /2.0f;
				else velocity.y = 0;
		...
		public override void Activate ()
		{
			...
			if(resetOnActivate){
				if(startingPositions.Length > 0)SetPosition(startingPositions[0].transform.position);
				else SetPosition(savedStartPosition);
			}
